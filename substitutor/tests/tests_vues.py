...
class TestsModels(TestCase):
    ...
    def setUp(self):
    	...
    	for product in self.validation.rows_products:
    		product_to_insert.append(Product.objects.create(code=product["code"],name=product["product_name"],quantite=product.get("quantity", ""),marque=product.get("brands", ""),nom_categories=product.get("categories", ""),labels=product.get("labels", ""),ingredients=product.get("ingredients_text", ""),nutriments=product["nutriments"],produits_provoqu_allergies=product["allergens_tags"],traces_eventuelles=product["traces_tags"],nutriscore=product["nutriscore_data"]["grade"],lien_o_ff=product.get("url", ""),url_image=product.get("image_url", ""), ))
    	...
    	for product_categorie in self.validation.rows_products_categories:
    		...
    		if ((self.validation.rows_products_categories.index(product_categorie)) % 500) == 0:
    ...
    def tests_vues(self):
    	...
    	request = self.factory.get("/substitutor/substitute/",{"un_suscribe_substitute_id": 2,"id_product": 1, "search_input":'nutella'},)
    	...
    	request = self.factory.get("/substitutor/substitute/", {"un_suscribe_substitute_id": 2,"id_product": 1} ,)
    	response = substitute(request)
